...
这是一个面向中小网购平台商家的本地可演示 MVP。商家上传商品表格和店铺规则后，系统会把资料保存到 SQLite，并用本地 embedding 模型写入 Chroma 向量库。顾客咨询商品推荐、尺码、材质等导购问题时，系统会优先做结构化商品筛选并返回商品卡片；咨询发货、退换货、售后等规则问题时，系统会做 RAG 检索，再调用 DeepSeek 生成简洁客服回复，并展示答案来源。
...
9. 进入“聊天记录”，确认刚才的问题、回答、mode 和人工处理状态已经保存。
10. 进入“未命中问题”，查看知识库无法回答的问题，并可标记为已处理。
11. 进入“顾客端预览”，用聊天窗口模拟真实顾客咨询。
...
10. 指出测试页的 `mode=deepseek_openai_compatible`，证明 DeepSeek 调用成功。
11. 进入“聊天记录”，切换筛选：
    - 全部
    - 需要人工处理
    - DeepSeek 成功
    - fallback
12. 进入“未命中问题”，展示“海外配送”或“100 元以内的包”等无法回答的问题，并演示标记为已处理。
13. 进入“顾客端预览”，询问 `我要找人工客服`，展示“已转人工处理”提示。
14. 总结系统边界：当前是本地 MVP，不做支付、不接真实电商平台、不做评论分析。
